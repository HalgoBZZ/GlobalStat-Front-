...
export class DebitComponent {
  ...
  ngOnInit() {
    ...
    this.nbTbon=0;
    this.nbTmauvais=0;
    this.questionService.getAllDebit().toPromise()
    .then( questions=> {
    ...
    localStorage.setItem("nbExcellentd",exc.toString());
    let tb=(this.nbTbon*100)/(this.nbUsers*this.nbQuest);
    localStorage.setItem("nbTbond",tb.toString());
    let moy=(this.nbMoyen*100)/(this.nbUsers*this.nbQuest);
    localStorage.setItem("nbMoyend",moy.toString());
    let mov=(this.nbMauvais*100)/(this.nbUsers*this.nbQuest);
    localStorage.setItem("nbMauvaisd",mov.toString());
    let tm=(this.nbTmauvais*100)/(this.nbUsers*this.nbQuest);
    localStorage.setItem("nbTmauvaisd",tm.toString());
  }); 
...
chart = new Chart({
    ...
    series: [
      {
        ...
        //data:[20,20,10,5]
        data: [
          parseInt(localStorage.getItem("nbTmauvaisd")),
          parseInt(localStorage.getItem("nbMauvaisd")),
          parseInt(localStorage.getItem("nbMoyend")),
          parseInt(localStorage.getItem("nbTbond")),
          parseInt(localStorage.getItem("nbExcellentd"))
        ]
      }
    ]
  });
  ...
  save(): void {
    this.questionService.createDebit(this.newQuest)
        .subscribe( data => {
